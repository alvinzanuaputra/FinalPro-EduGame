...
import { FSM, setSaldo, tambahUtang, setPJ, setKL, setHari } from './fsm.js?v=3';
import { render } from './render.js?v=3';
import { play, cont } from './dialog.js?v=3';
import { phoneJudol, phoneRegisterPinjol, phoneChatYadi } from './phone.js?v=3';
...
export const SCENES = {
  ...
  check_events: [
    {
      t: 'fx', run: () => {
        const hr = FSM.HR.s;
        if (hr === 0) play('terima2');
        else if (hr === 6) play('terima3');
        else if (hr === 17) play('jalurPinjol3');
        else if (hr === 23) play('jalur_30');
        else if (hr === 30) play('jalur_103');
        else play('interactive_loop');
      }
    }
  ],
  ...
  prologKayla: [
    ...
    { t: 'call', run: () => { import('./phone.js?v=3').then(p => p.phoneChatYadi(() => { import('./dialog.js?v=3').then(d => d.play('prologYadi')); })); } },
  ],
  prologYadi: [
  ...
  terima: [
    ...
    { t: 'say', who: 'Saya (Benny)', text: 'Lho, menang? Gampang banget ternyata. Naikin taruhan ah. (Klik HP di meja untuk main)', char: 'benny_happy.png' },
    { t: 'fx', run: () => { import('./main.js?v=3').then(m => m.enterInteractiveMode()); } },
  ],
  terima2: [
    ...
    { t: 'say', who: '', text: '— HARI KE-10 — Tabungan tinggal Rp 460.000.', char: 'benny_sad.png', bgm: 'Tense_BGM', sound: 'Whoosh_transition' },
    {
      t: 'call', run: () => {
        import('./phone.js?v=3').then(p => p.phoneNotification('BCA Mobile', 'Transfer Masuk: Rp 4.000.000<br><small>Keterangan: Gaji Bulan Ini</small>', () => {
          import('./dialog.js?v=3').then(d => d.next());
        }));
      }
    },
    { t: 'say', who: 'Saya (Benny)', text: 'Notifikasi gaji masuk! Pas banget buat tambahan modal judol hari ini.', char: 'benny_happy.png' },
  ...
  jalurPinjol: [
    ...
    {
      t: 'call', run: () => {
        import('./phone.js?v=3').then(p => p.phoneRegisterPinjol('Bangke - Pinjol', 'pinjol', () => {
          import('./dialog.js?v=3').then(d => d.play('jalurPinjol2'));
        }));
      }
    },
  ],
  jalurPinjol2: [
    ...
    { t: 'fx', run: () => { setHari(17); import('./main.js?v=3').then(m => m.enterInteractiveMode()); } },
  ],
  jalurPinjol3: [
  ...
  jalurPinjol4: [
    ...
    { t: 'fx', run: () => { setHari(23); import('./main.js?v=3').then(m => m.enterInteractiveMode()); } },
  ],
  jalur_30: [
